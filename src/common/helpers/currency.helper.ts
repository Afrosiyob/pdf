...
export default {
  format: (value: number, { separator = ' ', precision = 2, decimal = '.', symbol = '' } = {}) => {
  ...
  subtract: (value: currency.Any, subtractValue: currency.Any, { precision = 2 } = {}) =>
    String(currency(value, { precision }).subtract(subtractValue)),
  normalize: (value: currency.Any, { precision = 2 } = {}) => String(currency(value, { precision }))
};
